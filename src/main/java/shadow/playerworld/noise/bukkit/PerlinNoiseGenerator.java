...
extends NoiseGenerator {
    ...
    public double noise(double x, double y, double z) {
        int floorX = floor(x += this.offsetX);
        int floorY = floor(y += this.offsetY);
        int floorZ = floor(z += this.offsetZ);
        int X = floorX & 0xFF;
        int Y = floorY & 0xFF;
        int Z = floorZ & 0xFF;
        double fX = fade(x -= (double)floorX);
        double fY = fade(y -= (double)floorY);
        double fZ = fade(z -= (double)floorZ);
        int A = this.perm[X] + Y;
        ...
        return lerp(fZ, lerp(fY, lerp(fX, grad(this.perm[AA], x, y, z), grad(this.perm[BA], x - 1.0, y, z)), lerp(fX, grad(this.perm[AB], x, y - 1.0, z), grad(this.perm[BB], x - 1.0, y - 1.0, z))), lerp(fY, lerp(fX, grad(this.perm[AA + 1], x, y, z - 1.0), grad(this.perm[BA + 1], x - 1.0, y, z - 1.0)), lerp(fX, grad(this.perm[AB + 1], x, y - 1.0, z - 1.0), grad(this.perm[BB + 1], x - 1.0, y - 1.0, z - 1.0))));
    }
